(function (global, factory) {
  if (typeof exports === 'object' && typeof module !== 'undefined') module.exports = factory(global);else global.sf = factory(global);
})(typeof window !== "undefined" ? window : this, function (window) {
  'use strict';

  if (typeof document === undefined) document = window.document; // ===== Module Init =====
  ...
  function compareObject(obj1, obj2) {
    ...
    for (var i in obj1) {
      if (typeof obj1[i] !== 'object' && obj1[i] !== obj2[i]) return false;
    }
  ...
  sf.controller = new function () {
    ...
    self.modelScope = function (element, func) {
      ...
      if (!model) throw 'model or controller was not found';
      var bindedList = element.getAttribute('sf-bind-list');

      if (!bindedList) {
        var parentEl = $.parent(element, '[sf-bind-list]');
        if (parentEl !== null) bindedList = parentEl.getAttribute('sf-bind-list');
      } else var parentEl = element;

      if (!bindedList) {
        if (func) return func(sf.model.root[model], -1);else return sf.model.root[model];
      } // Find index


      var bindedListIndex = 0;
      if (bindedList) bindedListIndex = $.prevAll(parentEl, '[sf-bind-list]').length;
      if (func) return func(sf.model.root[model][bindedList], bindedListIndex);else return sf.model.root[model][bindedList][bindedListIndex];
    };

    self.modelName = function (element) {
      var name = undefined;
      if (element.hasAttribute('sf-controller')) name = element.getAttribute('sf-controller');else name = $.parent(element, '[sf-controller]').getAttribute('sf-controller'); // Initialize it first

      if (name !== undefined && !self.active[name]) self.run(name);
      return name;
    };

    var listenSFClick = function listenSFClick(e) {
      ...
      if (!sf.model.root[model]) throw "Couldn't find model for " + model + " that was called from sf-click";
      ...
      script = script.split('(');
      var method = script[0];
      var method_ = method; // Get method reference
      ...
      script = script.split(')');
      script.pop();
      script = script.join('('); // Turn argument as array
    ...
    var root_ = function root_(scope) {
      if (!sf.model.root[scope]) sf.model.root[scope] = {};
      if (!sf.model.root[scope]) sf.controller.run(scope);
      return sf.model.root[scope];
    };

    self.run = function (name, func) {
      if (!sf.loader.DOMWasLoaded) return sf(function () {
        self.run(name, func);
      });

      if (self.pending[name]) {
        if (!sf.model.root[name]) sf.model.root[name] = {};
        self.pending[name](sf.model.root[name], root_);
        self.active[name] = true;
        delete self.pending[name];
      }

      if (func) func(sf.model.root[name], root_);
    };
  ...
  (function () {
    ...
    self.parent = function (element, selector) {
      ...
      do {
        if (element.matches(selector) === true) return element;
    ...
    self.prevAll = function (element, selector, isNext) {
      var result = [];
      var findNodes = selector === null || selector.constructor !== String ? true : false; // Skip current element

      element = isNext ? element.nextSibling : element.previousSibling;

      while (element !== null) {
        if (findNodes === false && element.matches(selector) === true) result.push(element);else {
          if (element === selector) break;
          result.push(element);
        }
        if (isNext) element = element.nextSibling;else element = element.previousSibling;
      }
    ...
    self.on = function (element, event, selector, callback, once) {
      ...
      if (selector) {
        ...
        callback = function callback(ev) {
          if (self.parent(ev.target, selector) !== null) tempCallback(ev);
        };
      }
    ...
    self.off = function (element, event, selector) {
      ...
      if (ref !== undefined && ref[event] !== undefined) {
        for (var i = ref[event].length - 1; i >= 0; i--) {
          if (selector && ref[event][i].selector !== selector) continue;
    ...
    self.remove = function (elements) {
      if (elements.remove !== undefined) return elements.remove();
      ...
      while (element.parentNode !== null) {
        if (element.id) {
          ...
          if (element === untilElement) {
            if (childIndexes === undefined) names.unshift(element.tagName);else names.unshift(0);
          } else {
            var e = element;
            ...
            if (childIndexes) names.unshift(i);else names.unshift(":nth-child(" + i + ")");
          }
    ...
    self.childIndexes = function (array, context) {
      ...
      var i = 1;
      if (array[0].constructor === String) element = element.querySelector(array[0]);else if (array.length === 1) return element;
  ...
  */
  ...
  function Request(requestOptions) {
    ...
    if (options.dataType === 'json' && options.url.indexOf('callback=') >= 0) {
      ...
      script_1.src = requestUrl; // Handler
  ...
  function serializeQuery(params, prefix) {
    ...
    for (var i = 0; i < key.length; i++) {
      ...
      if (params.constructor === Array) key[i] += prefix + "[]";else if (params.constructor === Object) key[i] = prefix ? prefix + "[" + key[i] + "]" : key[i];
      if (typeof value === 'object') key[i] = serializeQuery(value, key[i]);else key[i] = key[i] + "=" + encodeURIComponent(value);
    }
  ...
  sf.loader = new function () {
    ...
    self.css = function (list) {
      if (self.DOMWasLoaded) {
        ...
        for (var i = list.length - 1; i >= 0; i--) {
          if ($('link[href*="' + list[i] + '"]').length !== 0) list.splice(i, 1);
        }
      ...
      for (var i = 0; i < list.length; i++) {
        ...
        s.rel = 'stylesheet';
        s.href = list[i];
        s.addEventListener('load', sf.loader.f, {
          once: true
        });
        s.addEventListener('error', sf.loader.f, {
          once: true
        });
    ...
    self.js = function (list) {
      if (self.DOMWasLoaded) {
        // check if some list was loaded
        for (var i = list.length - 1; i >= 0; i--) {
          if ($('script[src*="' + list[i] + '"]').length !== 0) list.splice(i, 1);
        }
    ...
    document.addEventListener("DOMContentLoaded", function (event) {
      ...
      if (isQueued === false && document.body) {
        ...
        for (var i = 0; i < isQueued.length; i++) {
          if (isQueued[i].nodeType === 1) isQueued[i].classList.add('sf-dom-queued');
        }
        ...
        for (var i = 0; i < temp.length; i++) {
          ...
          temp[i].addEventListener('load', sf.loader.f, {
            once: true
          });
          temp[i].addEventListener('error', sf.loader.f, {
            once: true
          });
        }
      }
      ...
      function onReadyState() {
        ...
        if (self.turnedOff === false && self.loadedContent < self.totalContent) return;
        if (/loaded|complete/.test(document.readyState) === false) return;
        clearInterval(onReadyState_timer);
        ...
        whenDOMReady = whenDOMLoaded = null; // Last init
  ...
  sf.model = function (scope) {
    if (!sf.model.root[scope]) sf.model.root[scope] = {};
    if (sf.controller.pending[scope]) sf.controller.run(scope);
    return sf.model.root[scope];
  };
  ...
  (function () {
    ...
    self.root = {};
    var processingElement = null; // For debugging, normalize indentation

    function trimIndentation(text) {
      var indent = text.split("\n", 3);
      if (indent[0][0] !== ' ' || indent[0][0] !== "\t") indent = indent[1];else indent = indent[0];
    ...
    var localEval = function localEval(script, _model_, _modelScope, _content_) {
      "use strict"; // ==== Security check ====

      var tempScript = script; // Remove quotes

      tempScript = tempScript.replace(sf.regex.getQuotes, '"Quotes"'); // Prevent vulnerability by remove bracket to avoid a function call
      ...
      while ((check_ = bracketMatch.exec(tempScript)) !== null) {
        ...
        if (allowedFunction.indexOf(check_[1]) === -1 && check_[1].split('.')[0] !== '_modelScope' && chackValidFunctionCall.test(check_[1][check_[1].length - 1])) {
      ...
      if (preventExecution) {
        ...
        console.log(trimIndentation(processingElement.outerHTML).trim()); //console.log(tempScript);
    ...
    self.modelKeys = function (modelRef) {
      ...
      for (var i = keys.length - 1; i >= 0; i--) {
        if (keys[i].indexOf('$') !== -1) keys.splice(i, 1);
      }
    ...
    function elseIfHandle(else_, scopes) {
      var elseIf = else_.elseIf; // Else if

      for (var i = 0; i < elseIf.length; i++) {
        // Check the condition
        scopes[0] = elseIf[i][0];
        if (!localEval.apply(self.root, scopes)) continue; // Get the value
      ...
      if (else_.elseValue === null) return '';
    ...
    var templateExec = function templateExec(parse, item) {
      var parsed = {}; // Get or evaluate static or dynamic data

      for (var i = 0; i < parse.length; i++) {
        var ref = parse[i];
        ref.data[1] = item; // Direct evaluation type

        if (ref.type === REF_DIRECT || ref.type === REF_EXEC) parsed[i] = {
          type: ref.type,
          data: localEval.apply(self.root, ref.data)
        }; // Conditional type
            ...
            scopes[0] = ref.if[0]; // If condition was not meet
    ...
    var templateParser = function templateParser(template, item) {
      var html = template.html.cloneNode(true);
      var addresses = template.addresses;
      var parsed = templateExec(template.parse, item); // Save model item reference to node

      html.model = item;
      var changesReference = [];
      var pendingInsert = []; // Find element where the data belongs to

      for (var i = 0; i < addresses.length; i++) {
        var ref = addresses[i];
        var current = $.childIndexes(ref.address, html); // Modify element attributes
        ...
        if (ref.nodeType === -1) {
          ...
          changesReference.push(cRef); // Pending element insert to take other element reference
      ...
      html.sf$changesReference = changesReference; // Run the pending element
    ...
    var dataParser = function dataParser(html, _model_, mask, scope, runEval, preParsedReference) {
      var _modelScope = self.root[scope];
      if (!runEval) runEval = ''; // Unmatch any function

      var variableList = self.modelKeys(_modelScope);

      for (var i = variableList.length - 1; i >= 0; i--) {
        if (_modelScope[variableList[i]] instanceof Function) variableList.splice(i, 1);
      } // Don't match text inside quote, or object keys


      var scopeMask = RegExp(sf.regex.strictVar + '(' + variableList + ')' + sf.regex.avoidQuotes + '\\b', 'g');
      if (mask) var itemMask = RegExp(sf.regex.strictVar + mask + '\\.' + sf.regex.avoidQuotes + '\\b', 'g');
      bindingEnabled = true;
      ...
      var prepared = html.replace(/{{([^@%][\s\S]*?)}}/g, function (actual, temp) {
        ...
        if (mask) temp = temp.replace(itemMask, function (matched) {
        ...
        temp = temp.split('&amp;').join('&').split('&lt;').join('<').split('&gt;').join('>'); // Evaluate

        if (runEval === '#noEval') {
          temp = temp.trim(); // Simplicity similar
    ...
    var uniqueDataParser = function uniqueDataParser(html, _model_, mask, scope, runEval) {
      // Get prepared html content
      var _content_ = {
        length: 0,
        take: function take(passVar, currentIndex) {
          if (passVar === null) return dataParser(this[currentIndex], _model_, mask, scope); // Use strict mode and prepare for new variables

          var strDeclare = '"use strict";var ';
          var firstTime = true; // Declare new variable

          for (var key in passVar) {
            if (typeof passVar[key] === 'string') passVar[key] = '"' + passVar[key].split('"').join('\\"') + '"';else if (typeof passVar[key] === 'object') passVar[key] = JSON.stringify(passVar[key]);
            if (!firstTime) strDeclare += ',';
            strDeclare += key + ' = ' + passVar[key];
            firstTime = false;
          } // Remove var because no variable are being passed


          if (firstTime === true) strDeclare = strDeclare.replace('var ', ''); // Escape function call for addional security eval protection

          strDeclare = strDeclare.split('(').join('&#40;').split(')').join('&#41;'); // Pass to static data parser for another HTML data
      ...
      var prepared = html.replace(/{{(@[\s\S]*?)}}/g, function (actual, temp) {
        ...
        var result = '';
        var check = false; // Get defined variables
        ...
        if (_model_ === null && runEval === '#noEval') VarPass.push('_model_');
        ...
        if (VarPass.length !== 0) {
          ...
          if (VarPass.length === 0) VarPass = 'null';else VarPass = '{' + VarPass.join(',') + '}';
        ...
        if (check.length !== 1) {
          check = check[1].split(':'); // {if, elseIf:([if, value], ...), elseValue}

          var findElse = function findElse(text) {
            text = text.join(':');
            var else_ = null; // Get else value

            var text = text.split('@else' + (text.indexOf(':') !== -1 ? ':' : ' :'));
            if (text.length === 2) else_ = text.pop();else text = text[0]; // Split elseIf
          ...
          if (runEval === '#noEval') {
            ...
            elseIf.type = REF_IF;
            elseIf.data = [null, _model_, _modelScope, _content_]; // Trim Data

            elseIf.if = [condition.trim(), elseIf.if.trim()];
            if (elseIf.elseValue !== null) elseIf.elseValue = elseIf.elseValue.trim();
          ...
          var scopes = [check[0], _model_, _modelScope, _content_]; // If condition was not meet
    ...
    var bindArray = function bindArray(template, list, mask, modelName, propertyName, targetNode, parentNode, tempDOM) {
      var editProperty = ['pop', 'push', 'splice', 'shift', 'unshift', 'swap', 'move', '$replace', 'softRefresh', 'hardRefresh'];
      var refreshTimer = -1;

      var processElement = function processElement(index, options, other, count) {
        if (options === 'clear') {
          if (list.$virtual) var spacer = [parentNode.firstElementChild, parentNode.lastElementChild];
        ...
        if (options === 'hardRefresh') {
          ...
          for (var i = index; i < item.length; i++) {
            if (list.$virtual) parentNode.insertBefore(templateParser(template, item[i]), parentNode.lastElementChild);else parentNode.appendChild(templateParser(template, item[i]));
          }
        ...
        if (options === 'swap' || options === 'move') {
          ...
          if (options === 'move') {
            ...
            if (overflow < 0) count += overflow; // Move to virtual DOM
            ...
            var nextSibling = ref[other] || null; // Move to defined index

            for (var i = 0; i < count; i++) {
              parentNode.insertBefore(vDOM.firstElementChild, nextSibling);
              if (callback !== undefined && callback.update) callback.update(nextSibling !== null && nextSibling.previousElementSibling || parentNode.lastElementChild, 'move');
            }
        ...
        } else exist = parentNode.children; // Remove
        ...
        if (options === 'remove') {
          if (exist[index]) {
            ...
            var startRemove = function startRemove() {
            ...
            if (callback !== undefined && callback.remove) {
              ...
              if (!callback.remove(exist[index], startRemove)) setTimeout(startRemove, 800);
          ...
          return;
            ...
            if (!other) other = index + 1;else if (other < 0) other = list.length + other;else other = index - other;

            for (var i = index; i < other; i++) {
              var oldChild = exist[i];
              if (oldChild === undefined || list[i] === undefined) break;
              ...
              if (callback !== undefined && callback.update) callback.update(temp, 'replace');
            }
          }
        ...
        var referenceNode = exist[index]; // Create
        ...
        if (options === 'insertAfter') {
          ...
          if (!referenceNode) {
            if (!list.$virtual || list.length === 0) {
              ...
              if (callback !== undefined && callback.create) callback.create(temp);
            }
          ...
          if (callback !== undefined && callback.create) callback.create(temp); // Refresh virtual scroll
      ...
      var propertyProxy = function propertyProxy(subject, name) {
        Object.defineProperty(subject, name, {
          ...
          value: function value() {
            ...
            if (Array.prototype[name]) temp = Array.prototype[name].apply(this, arguments);
            if (name === 'pop') processElement(this.length, 'remove');else if (name === 'push') processElement(lastLength, 'append');else if (name === 'shift') processElement(0, 'remove');else if (name === 'splice') {
              if (arguments[0] === 0 && arguments[1] === undefined) return temp; // Removing data
            ...
            } else if (name === 'unshift') processElement(0, 'insertBefore');else if (name === 'softRefresh') processElement(arguments[0], 'update', arguments[1]);else if (name === 'hardRefresh') processElement(0, 'hardRefresh');
            return temp;
          }
        });
      };
      ...
      if (parentNode && parentNode.classList.contains('sf-virtual-list')) {
        delete list.$virtual;
        list.$virtual = {}; // Transfer virtual DOM
      ...
      Object.defineProperty(list, 'getElement', {
        ...
        value: function value(index) {
          if (list.$virtual) {
            if (index < list.$virtual.DOMCursor) return list.$virtual.dom.children[index];
            index -= list.$virtual.DOMCursor;
            var childElement = parentNode.childElementCount - 2;
            if (index <= childElement) return parentNode.children[index + 1];
    ...
    var loopParser = function loopParser(name, template, script, targetNode, parentNode) {
      var method = script.split(' in ');
      var mask = method[0];
      if (!self.root[name]) return console.error("Can't parse element because model for '" + name + "' was not found", template);
      ...
      template.setAttribute('sf-bind-list', method[1]); // Get reference for debugging
      ...
      if (method.length === 2) {
        ...
        if (modelRef.sf$bindedKey === undefined) initBindingInformation(modelRef);
        if (modelRef.sf$bindedKey[method[1]] === undefined) modelRef.sf$bindedKey[method[1]] = null;
        Object.defineProperty(modelRef, method[1], {
          ...
          set: function set(val) {
            if (val.length === 0) return items.splice(0);
            return items.$replace(val);
          }
        });
        bindArray(template, items, mask, name, method[1], targetNode, parentNode, tempDOM); // Output to real DOM if not being used for virtual list
    ...
    var inputBoundFunction = function inputBoundFunction(e) {
      self.root[e.target['sf-model']][e.target['sf-bounded']] = e.target.value;
    };

    var bindInput = function bindInput(targetNode) {
      ...
      for (var i = 0; i < temp.length; i++) {
        ...
        var model = sf.controller.modelName(element);
        if (!model) return;
        var whichVar = element.getAttribute('sf-bound'); // Get reference
        ...
        element.removeAttribute('sf-bound'); // Bound value change

        if (element.tagName === 'INPUT' || element.tagName === 'TEXTAREA') $.on(element, 'keyup', inputBoundFunction);else $.on(element, 'change', inputBoundFunction);
      }
    };
    ...
    self.init = function (targetNode, queued) {
      ...
      for (var i = 0; i < temp.length; i++) {
        ...
        if (queued !== undefined) element.classList.remove('sf-dom-queued');
        ...
        if (element.parentNode.classList.contains('sf-virtual-list')) {
          ...
          ceiling.classList.add('ceiling'); //ceiling.style.transform = 'scaleY(0)';

          element.parentNode.insertAdjacentElement('afterBegin', ceiling); // prepend

          floor.classList.add('floor'); //floor.style.transform = 'scaleY(0)';
          ...
          styles = null; // Element height + margin

          absHeight = Math.ceil(element.offsetHeight + absHeight);
        }

        var after = element.nextElementSibling;
        if (after === null || element === after) after = false;
        var before = element.previousElementSibling;
        if (before === null || element === before) before = false;
        var script = element.getAttribute('sf-repeat-this');
        element.removeAttribute('sf-repeat-this');
        var controller = sf.controller.modelName(element); // Check if the element was already bound to prevent vulnerability

        if (/sf-bind-key|sf-bind-list/.test(element.outerHTML)) throw "Can't parse element that already bound";
    ...
    function DOMNodeRemoved(element) {
      ...
      if (element.hasAttribute('sf-controller') === false) return;
      removeModelBinding(element.getAttribute('sf-controller'));
    }

    sf(function () {
      var everyRemovedNodes = function everyRemovedNodes(nodes) {
      ...
      if (typeof MutationObserver === 'function' && MutationObserver.prototype.observe) {
        var everyRecords = function everyRecords(record) {
    ...
    var removeModelBinding = function removeModelBinding(modelName) {
      ...
      for (var key in bindedKey) {
        ...
        if (ref[key].constructor === String || ref[key].constructor === Number || ref[key].constructor === Boolean) {
          /* Ok */
        ...
        if (Object.getOwnPropertyDescriptor(ref, key) === undefined) continue; // Reconfigure / Remove property descriptor
    ...
    var bindObject = function bindObject(element, modelRef, propertyName, which) {
      if (!(element instanceof Node)) element = element[0]; // Get reference for debugging

      processingElement = element; // First initialization

      element.setAttribute('sf-bind-key', propertyName);
      var modelName = sf.controller.modelName(element); // Cache attribute content

      if (which === 'attr' || !which) {
        var attrs = {};

        for (var i = 0; i < element.attributes.length; i++) {
          var attr = element.attributes[i].name; // Check if it has a bracket

          if (dcBracket.test(element.getAttribute(attr)) === false) continue;
      ...
      if (which === 'html' || !which) var htmlClone = element.cloneNode(true).innerHTML;

      var onChanges = function onChanges() {
        if (which === 'attr' || !which) {
          for (var name in attrs) {
            if (attrs[name].indexOf(propertyName) === -1) continue;
            var temp = dataParser(attrs[name], modelRef, false, modelName);
            if (name === 'value') element.value = temp;else element.setAttribute(name, temp);
            break;
          }
        }
      ...
      if (modelRef[propertyName] === undefined) throw "Property '" + propertyName + "' was not found on '" + modelName + "' model"; // Enable multiple element binding
    ...
    self.bindElement = function (element, which) {
      ...
      var html = element.outerHTML; // Check if the child element was already bound to prevent vulnerability
      ...
      if (which === 'attr') html = html.replace(element.innerHTML, '');
      var brackets = /{{([\s\S]*?)}}/g; // Unmatch any function

      var variableList = self.modelKeys(model);

      for (var i = variableList.length - 1; i >= 0; i--) {
        if (model[variableList[i]] instanceof Function) variableList.splice(i, 1);
      }
    ...
    self.extractPreprocess = function (targetNode, mask, name) {
      var copy = targetNode.outerHTML; // Mask the referenced item

      copy = copy.split('#' + mask).join('_model_'); // Extract data to be parsed

      copy = uniqueDataParser(copy, null, mask, name, '#noEval');
      var preParsed = copy[1];
      copy = dataParser(copy[0], null, mask, name, '#noEval', preParsed); // Build element and start addressing
      ...
      function addressAttributes(currentNode) {
        ...
        for (var a = 0; a < attrs.length; a++) {
          ...
          if (found.length !== 1) {
            ...
            if (found[0] === '' && found.length === 2) key.direct = Number(found[1]) || false;
            keys.push(key);
          }
        }
      ...
      if (currentElement.length !== 0) addressed.push({
        nodeType: 1,
        address: [0],
      ...
      for (var i = 0; i < nodes.length; i++) {
        ...
        if (temp.nodeType === 1) {
          ...
          if (temp.indexes.length !== 0) {
            ...
            if (temp.indexes.length !== 0 && nodes[i].textContent.length !== 0) addressStart = $.getSelector(nodes[i], true); // parent.insertBefore(document.createComment('sf_parse_start'), nextSibling);
            ...
            for (var a = 0; a < temp.indexes.length; a++) {
              ...
              if (innerHTML[a]) {
                ...
                parent.insertBefore(textNode, nextSibling); // Get new start flag

                if (a + 1 < temp.indexes.length) addressStart = $.getSelector(textNode, true); // if(nodes[i].previousSibling.nodeType !== 8 || nodes[i].previousSibling.textContent.indexOf('sf_parse_') === -1)
            ...
            if (nodes[i].textContent === '') {
              ...
              continue;
            } else if (nodes[i].textContent.search(/{{%=[0-9]+/) === -1) continue;
            ...
            if (innerHTML[0] === '' && innerHTML.length === 2) temp.direct = Number(innerHTML[1]) || false;
            temp.address = $.getSelector(nodes[i], true);
          }
        }
    ...
    self.queuePreprocess = function (targetNode, extracting) {
      ...
      for (var i = childNodes.length - 1; i >= 0; i--) {
        ...
        if (extracting === undefined && excludes.indexOf(currentNode.nodeName) !== -1) continue;

        if (currentNode.nodeType === 1) {
          // Tag
          var attrs = currentNode.attributes; // Skip element and it's childs that already bound to prevent vulnerability

          if (attrs['sf-bind-key'] || attrs['sf-repeat-this'] || attrs['sf-bind-list']) continue;

          for (var a = 0; a < attrs.length; a++) {
            if (attrs[a].value.indexOf('{{') !== -1) {
              if (extracting === undefined) currentNode.setAttribute('sf-preprocess', 'attronly');
          ...
          if (currentNode.nodeValue.indexOf('{{') !== -1) {
            if (extracting === undefined) {
              currentNode.parentNode.setAttribute('sf-preprocess', ''); // Reset Siblings
    ...
    self.parsePreprocess = function (nodes, queued) {
      for (var a = 0; a < nodes.length; a++) {
        ...
        if (queued !== undefined) current.classList.remove('sf-dom-queued');
        if (!self.root[model]) return console.error("Can't parse element because model for '" + model + "' was not found", current);
        var modelRef = self.root[model]; // Double check if the child element already bound to prevent vulnerability
        ...
        if (current.hasAttribute('sf-bind')) self.bindElement(current, current.getAttribute('sf-bind')); // Avoid editing the outerHTML because it will remove the bind
    ...
    function initBindingInformation(modelRef) {
      if (modelRef.sf$bindedKey !== undefined) return; // Element binding data
  ...
  sf.router = new function () {
    ...
    var currentRouterURL = ''; // Should be called if not using lazy page load
    ...
    self.init = function (targetNode) {
      ...
      for (var i = 0; i < temp.length; i++) {
        if (temp[i].getAttribute('sf-controller')) sf.controller.run(temp[i].getAttribute('sf-controller'));
    ...
    self.enable = function (status) {
      ...
      if (status === true) {
        ...
        $.on(document.body, 'click', 'a[href]', self.load); // Create listener when navigate backward
    ...
    var before = {}; // Set index with number if you want to replace old function

    self.before = function (name, func, index) {
      if (!before[name]) before[name] = [];

      if (index === undefined) {
        if (before[name].indexOf(func) === -1) before[name].push(func);
      } else before[name][index] = func;
    };

    var after = {}; // Set index with number if you want to replace old function

    self.after = function (name, func, index) {
      if (!after[name]) after[name] = [];

      if (index === undefined) {
        if (after[name].indexOf(func) === -1) after[name].push(func);
      } else after[name][index] = func;
    };
    ...
    var beforeEvent = function beforeEvent(name) {
      if (self.currentPage.indexOf(name) === -1) self.currentPage.push(name);
    ...
    var afterEvent = function afterEvent(name) {
      if (self.currentPage.indexOf(name) === -1) self.currentPage.splice(self.currentPage.indexOf(name), 1);
    ...
    self.on = function (event, func) {
      if (onEvent[event].indexOf(func) === -1) onEvent[event].push(func);
    };
    ...
    */
    ...
    self.load = function (ev) {
      ...
      var elem = ev.target;
      if (!elem.href) return;
      if (!history.pushState || elem.hasAttribute('sf-router-ignore')) return; // Make sure it's from current origin
    ...
    self.goto = function (path, data, method) {
      ...
      RouterLoading = $.ajax({
        ...
        success: function success(data) {
          ...
          lazyRouting = true; // Run 'loaded' event

          RouterLoading = false; // Find special data
          ...
          var foundAction = function foundAction(ref) {
            DOMReference = $.findOne(ref); // Run 'after' event for old page view

            var last = $.findOne('[sf-page]', DOMReference);
            afterEvent(last ? last.getAttribute('sf-page') : '/'); // Redefine title if exist

            if (special && special.title) $('head > title').innerHTML = special.title;
            found = true;
          };
          ...
          if (!found) {
            ...
            if (sf.router.lazyViewPoint["@default"]) foundAction(sf.router.lazyViewPoint["@default"]);
          ...
          if (self.pauseRenderOnTransition) self.pauseRenderOnTransition.css('display', 'none'); // Let page script running first, then update the data binding

          DOMReference.innerHTML = data; // Parse the DOM data binding

          sf.model.init(DOMReference); // Init template to model binding
          ...
          if (self.pauseRenderOnTransition) self.pauseRenderOnTransition.css('display', '');
      ...
      if (!routingBack) window.history.pushState(null, "", path);
      routingBack = false;
      return true;
  ...
  sf.internal.virtual_scroll = new function () {
    ...
    var scrollingByScript = false; // before and after
    ...
    self.handle = function (list, targetNode, parentNode) {
      ...
      list.$virtual.scrollHeight = list.$virtual.dCursor.floor.offsetTop - list.$virtual.dCursor.ceiling.offsetTop;
      var scroller = null;
      ...
      setTimeout(function () {
        ...
        if (parentNode.classList.contains('sf-list-dynamic')) dynamicHeight(list, targetNode, parentNode, scroller);else staticHeight(list, targetNode, parentNode, scroller);
      }, 500);
    ...
    function dynamicHeight(list, targetNode, parentNode, scroller) {
      ...
      function previousCeiling() {
        var temp = null;
        var resetCeiling = false; // Add some element on the ceiling

        for (var i = 0; i < self.prepareCount; i++) {
          if (vCursor.floor === null) temp = virtual.dom.lastElementChild;else temp = vCursor.floor.previousElementSibling;
          if (temp === null) break;
          vCursor.ceiling = temp.previousElementSibling;
          virtual.DOMCursor--;
          ceiling.insertAdjacentElement('afterEnd', temp);
          if (ceilingHeight > 0) ceilingHeight -= getAbsoluteHeight(temp);
        ...
        if (ceilingHeight < 0 || temp === null) ceilingHeight = 0;
        var length = parentNode.childElementCount - 2 - list.$virtual.preparedLength; // Remove some element on the floor

        for (var i = 0; i < length; i++) {
          temp = floor.previousElementSibling;
          floorHeight += getAbsoluteHeight(temp);
          if (vCursor.floor === null) virtual.dom.insertAdjacentElement('beforeEnd', temp);else vCursor.floor.insertAdjacentElement('beforeBegin', temp);
          vCursor.floor = temp;
        }

        if (vCursor.floor === null) vCursor.ceiling = virtual.dom.lastElementChild;else vCursor.ceiling = vCursor.floor.previousElementSibling;
      ...
      function fillViewport() {
        ...
        for (var i = 0; i < length; i++) {
          if (vCursor.ceiling === null) temp = virtual.dom.firstElementChild;else temp = vCursor.ceiling.nextElementSibling;
          if (temp === null) break;
          vCursor.floor = temp.nextElementSibling;
          floor.insertAdjacentElement('beforeBegin', temp);
        }
      ...
      function nextFloor() {
        var temp = null;
        fillViewport();
        if (vCursor.ceiling === null) vCursor.ceiling = vCursor.floor.previousElementSibling; // Add extra element based on prepare count
        ...
        for (var i = 0; i < self.prepareCount; i++) {
          ...
          vCursor.floor = temp.nextElementSibling;
          floor.insertAdjacentElement('beforeBegin', temp);
          if (floorHeight > 0) floorHeight -= getAbsoluteHeight(temp);
        }

        if (floorHeight < 0 || temp === null) floorHeight = 0; // Remove some element on the ceiling
        ...
        for (var i = 0; i < length; i++) {
          ...
          virtual.DOMCursor++;
          if (vCursor.ceiling === null) virtual.dom.insertAdjacentElement('afterBegin', temp);else vCursor.ceiling.insertAdjacentElement('afterEnd', temp);
          vCursor.ceiling = temp;
        }

        if (vCursor.ceiling === null) vCursor.floor = virtual.dom.firstElementChild;else vCursor.floor = vCursor.ceiling.nextElementSibling;
        ceiling.style.height = ceilingHeight + 'px';
        floor.style.height = floorHeight + 'px';
      }
      ...
      function checkCursorPosition() {
        ...
        if (scroller.scrollTop < bounding.ceiling) {
          ...
          previousCeiling();
          refreshScrollBounding(virtual.DOMCursor, bounding, list, parentNode); // console.warn('back', bounding, scroller.scrollTop, virtual.DOMCursor);
        } else if (scroller.scrollTop > bounding.floor) {
          // console.log('front', bounding, scroller.scrollTop, virtual.DOMCursor);
          nextFloor();
          refreshScrollBounding(virtual.DOMCursor, bounding, list, parentNode); // console.warn('front', bounding, scroller.scrollTop, virtual.DOMCursor);
        }
    ...
    function staticHeight(list, targetNode, parentNode, scroller) {
      var virtual = list.$virtual;
      var ceiling = virtual.dCursor.ceiling;
      var floor = virtual.dCursor.floor; // Insert visible element to dom tree

      var insertCount = virtual.preparedLength <= list.length ? virtual.preparedLength : list.length;

      for (var i = 0; i < insertCount; i++) {
        if (virtual.dom.firstElementChild === null) break;
      ...
      function checkCursorPosition() {
        ...
        if (cursor + virtual.preparedLength > list.length) cursor = list.length - virtual.preparedLength;

        if (fromCeiling) {
          if (cursor < self.prepareCount * 2) cursor -= self.prepareCount; // Fix chrome scroll anchoring bugs
        ...
        if (cursor + changes >= list.length) changes = cursor + changes - list.length;

        if (changes === 0) {
          // This should be fixed to improve performance and future bugs
          //console.warn("No changes (The scroll bounding is not correct)");
          updating = false;
          return;
        }

        virtual.DOMCursor = cursor; //console.log(cursor, changes);
        ...
        refreshScrollBounding(cursor, bounding, list, parentNode); //console.log('a', bounding.ceiling, bounding.floor, scroller.scrollTop);

        updating = false;
      }

      $.on(scroller, 'scroll', checkCursorPosition); // For preventing scroll jump if scrolling over than viewport
    ...
    function refreshScrollBounding(cursor, bounding, list, parentNode) {
      ...
      if (cursor < self.prepareCount) {
        ...
        if (bounding.floor !== undefined) bounding.floor = bounding.floor.offsetTop;else bounding.floor = parentNode.lastElementChild.offsetTop + 1000;
        return;
      } else if (parentNode.children[temp + 1] !== undefined) bounding.ceiling = parentNode.children[temp + 1].offsetTop; // -2 element


      if (list.$virtual.preparedLength !== undefined && cursor >= list.length - list.$virtual.preparedLength) bounding.floor = list.$virtual.dCursor.floor.offsetTop + list.$virtual.scrollHeight * 2;else {
    ...
    function moveElementCursor(changes, list) {
      ...
      if (changes > 0) {
        // forward
        var ref = 0; // Select from virtual ceiling cursor to Dom tree

        for (var i = 0; i < changes; i++) {
          // vDom -> Dom tree
          if (vCursor.ceiling === null) ref = vDOM.firstElementChild;else ref = vCursor.ceiling.nextElementSibling;
          dCursor.floor.insertAdjacentElement('beforeBegin', ref);
        } // Move element on the ceiling to vDom


        for (var i = changes; i > 0; i--) {
          // Dom tree -> vDom
        ...
        // backward
        var ref = 0;
        changes = -changes; // Select from virtual floor cursor to Dom tree

        for (var i = 0; i < changes; i++) {
          // vDom -> Dom tree
          if (vCursor.floor === null) ref = vDOM.lastElementChild;else ref = vCursor.floor.previousElementSibling;
    ...
    function scrollTo(index, list, prepareCount, parentNode, scroller, refreshVirtualSpacer) {
      ...
      scrollingByScript = true; // Already on DOM tree

      if (virtual.DOMCursor === 0 && index < prepareCount + prepareCount / 2 || virtual.DOMCursor + prepareCount / 2 > index && virtual.DOMCursor + prepareCount < index) scroller.scrollTop = parentNode.children[index - virtual.DOMCursor + 1].offsetTop; // Move cursor
      else {
          var temp = null;
          var ceiling = virtual.dCursor.ceiling;
          var floor = virtual.dCursor.floor;
          var vCursor = virtual.vCursor; // DOM tree to virtual DOM
          ...
          for (var i = 0; i < length; i++) {
            ...
            if (vCursor.floor === null) {
              ...
              if (i === length - 1) vCursor.floor = temp;
            } else vCursor.floor.insertAdjacentElement('beforeBegin', temp);
          }

          if (index >= prepareCount) {
            if (index < list.length - virtual.preparedLength) index -= prepareCount;
          } else {
            reduce = prepareCount - index;
            virtual.DOMCursor = index = 0;
          }

          var insertCount = virtual.preparedLength <= list.length ? virtual.preparedLength : list.length; // Virtual DOM to DOM tree
          ...
          if (refreshVirtualSpacer) refreshVirtualSpacer(index);
          refreshScrollBounding(index, virtual.bounding, list, parentNode);
          temp = parentNode.children[prepareCount - reduce + 1];
          if (temp !== undefined) scroller.scrollTop = temp.offsetTop - scroller.offsetTop;
        }
    ...
    function refresh(force, list, prepareCount, parentNode, scroller, checkCursorPosition, refreshVirtualSpacer) {
      ...
      var additionalScroll = 0; // Find nearest cursor for current view position
      ...
      if (force || parentNode.childElementCount - 2 > list.$virtual.preparedLength) {
        ...
        var moveTo = cursor;
        if (!force) moveTo = cursor <= prepareCount ? cursor : cursor + prepareCount;
        scrollTo(moveTo, list, prepareCount, parentNode, scroller, refreshVirtualSpacer);
        scroller.scrollTop += additionalScroll;
      }

      if (refreshVirtualSpacer) refreshVirtualSpacer(cursor);
      if (checkCursorPosition) checkCursorPosition();
      ...
      if (_onElementResize_timer === -1) {
        _onElementResize_timer = setInterval(function () {
          ...
          for (var i = _onElementResize.length - 1; i >= 0; i--) {
            temp = _onElementResize[i]; // Check resize

            if (temp.element.scrollHeight === temp.height || temp.element.scrollWidth === temp.width) continue; // Check if it's removed from DOM
    ...
    function offElementResize(parentNode) {
      for (var i = _onElementResize.length - 1; i >= 0; i--) {
        if (_onElementResize[i].element === parentNode) _onElementResize.splice(i, 1);
    ...
    function initStyles() {
      ...
      style.sheet.insertRule('.sf-virtual-list .virtual-spacer{' + 'visibility: hidden;' + 'position: relative;' + 'height: 1px;' + 'transform-origin: 0 0;' + 'width: 1px;' + 'margin: 0;' + 'padding: 0;' + 'background: none;' + 'border: none;' + 'box-shadow: none;' + '}', style.sheet.cssRules.length);
    }
  }();
  return sf; // ===== Module End =====
});